...
func (sf *sftpFinder) OpenUpload(ctx context.Context, remoteDir, remoteFile string) (UploadSession, error) {
	...
	// 构建路径并确保目录存在
	finalPath := normalizePath(filepath.Join(actualRemoteDir, remoteFile))
	partPath := finalPath + ".part"
	dir := filepath.Dir(finalPath)
	if _, err := sf.client.Stat(dir); os.IsNotExist(err) {
		if err = sf.client.MkdirAll(dir); err != nil {
			return nil, err
		}
	}

	sess := &sftpUploadSession{
		client:    sf.client,
		partPath:  partPath,
		finalPath: finalPath,
	}
	// 延迟打开文件，第一次 WriteAt 时再打开，可避免无效句柄占用
	return sess, nil
}
...
// OpenRead 打开远程文件并返回可随机读取的视图，用于流式下载/断点续传
func (sf *sftpFinder) OpenRead(ctx context.Context, filePath string) (ra io.ReaderAt, size int64, modTime time.Time, name string, closer io.Closer, err error) {
	// 解析路径为实际文件系统路径
	actualPath := parseVueFinderPath(filePath)
